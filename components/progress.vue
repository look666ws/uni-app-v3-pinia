<template>
	<view class="wrapper" :style="{width: `${ widthWrapper }rpx`}">
		...
		<view :style="{left: `${ widthInner - 20 }rpx`}" class="circleWrapper" @touchmove="touchmove">
...
<script setup>
	...
	const touchmove = (e) => {
		...
		newX = newX < 0 ? 0 : newX
		widthInner.value = widthInner.value <= widthWrapper.value ? Math.round(newX) : widthWrapper.value
	}
</script>
